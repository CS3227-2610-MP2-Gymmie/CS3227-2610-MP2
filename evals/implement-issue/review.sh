...
Review the diff from merge base $merge_base (base ref $base_ref) to the current working tree, including uncommitted changes. Do not edit files.
...
codex exec --sandbox read-only -o "$review_file" "$prompt"
{ cat "$review_file"; printf '\n\nReviewed: %s (base: %s)\n' "$(date +%F)" "$base_ref"; } > "$body_file"
