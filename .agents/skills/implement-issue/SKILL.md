...
5. **Verify.** Run the repository's own checks suited to the criteria against the final relevant code state. Add interface checks when affected behavior warrants them. If a necessary check cannot run, try a suitable in-scope alternative when available. Rerun checks when later edits could affect their result. Report unavailable evidence; never claim an unsupported pass. After verification passes, run the repository's independent review command if one is provided. For each blocking finding, fix it and rerun verification, or record why you rejected it. Rerun the review until it reports Overall: pass, for at most 3 rounds. Then report the result and any remaining findings to the user, who has the final say. Never edit the review's output.
